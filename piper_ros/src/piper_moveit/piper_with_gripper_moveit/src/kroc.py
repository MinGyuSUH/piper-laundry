...
class ControlTower(Node):
    ...
    def __init__(self):
        ...
        self.ba1 = [1.808820692, 2.61520448, -1.3734707840000002, 0.0, 0.368644052, 0.7130060560000001, 0.0, 0.0]
        self.ba1_up = [1.7909405920000003,1.916799052,-1.5689656920000001,0.03528921200000001,1.226941184,0.334628252, 0.0,0.0]
        self.ba2 = [-1.5783331200000001, 1.568843584000, -2.34593889600, -0.34423989600, 0.9140656, 0.218957088, 0.0, 0.0]
        self.edge =[-1.5765015, 0.6961028200000001,-1.05480379200,-0.131510316000,0.357305452,0.254385852, 0.0,0.0]
        self.edge_up = [-1.48858374,0.618756124,-1.2031824560000002,-0.05371007600000001,0.6170815000000001,0.336651756, 0.0,0.0]

    ###############################################################################################    
    def _ox(self, v):
        """True/False/None -> 'O'/'X'/'-' (why: 가독성)"""
        if v is True: return "O"
        if v is False: return "X"
        return "-"

    def _current_state_table(self) -> str:
        """현재 플래그로 O/X 표 문자열 생성"""
        rows = [
            ("Heuristic", self.z_aligned,   self.contacted),
            ("KMeans",    self.z_aligned_k if getattr(self, "kmeans_available", False) else None,
                        self.contacted_kk if getattr(self, "kmeans_available", False) else None),
            ("FCM",       self.z_aligned_f if getattr(self, "fcm_available", False) else None,
                        self.contacted_ff if getattr(self, "fcm_available", False) else None),
        ]
        col1_w = max(len("Method"), *(len(r[0]) for r in rows))
        c2_w, c3_w = len("z_aligned"), len("contacted")

        top = f"┌{'─'*(col1_w+2)}┬{'─'*(c2_w+2)}┬{'─'*(c3_w+2)}┐"
        mid = f"├{'─'*(col1_w+2)}┼{'─'*(c2_w+2)}┼{'─'*(c3_w+2)}┤"
        bot = f"└{'─'*(col1_w+2)}┴{'─'*(c2_w+2)}┴{'─'*(c3_w+2)}┘"
        header = f"│ {'Method'.ljust(col1_w)} │ {'z_aligned'.ljust(c2_w)} │ {'contacted'.ljust(c3_w)} │"

        body = []
        for name, za, ct in rows:
            body.append(f"│ {name.ljust(col1_w)} │ {self._ox(za).ljust(c2_w)} │ {self._ox(ct).ljust(c3_w)} │")

        return "\n".join([top, header, mid, *body, bot])

    def _log_state_table(self, ctx: str = "") -> None:
        """표를 로그로 출력"""
        prefix = f"[STATE:{ctx}]\n" if ctx else ""
        self.get_logger().info(prefix + self._current_state_table())
        self.get_logger().info(f"norm: {self.norm}, theta: {self.theta}")


    def _load_minmax(self, path: str):
        """taxel_minmax_norm.npy -> (gmin(48,), grng(48,)) as float64"""
        try:
            mm = np.load(path, allow_pickle=True).item()
            gmin = np.asarray(mm["min"], dtype=np.float64)
            grng = np.asarray(mm["range"], dtype=np.float64)
            # 안전가드
            if gmin.shape[0] != 48 or grng.shape[0] != 48:
                raise ValueError(f"min/range shape must be 48, got {gmin.shape}, {grng.shape}")
            grng[~np.isfinite(grng) | (grng <= 0.0)] = 1.0
            return gmin, grng
        except Exception as e:
            self.get_logger().warn(f"MinMax load failed ({e}); using identity (no-op)")
            gmin = np.zeros(48, dtype=np.float64)
            grng = np.ones(48, dtype=np.float64)
            return gmin, grng

    def _normalize_taxels(self, x_vals, y_vals, z_vals):
        """
        raw 16×(x,y,z) -> 48축 Min–Max 정규화 결과를 분해해서 (x,y,z) 반환 (모두 float64)
        """
        A = np.empty(48, dtype=np.float64)
        A[0:16] = np.asarray(x_vals, dtype=np.float64)
        A[16:32] = np.asarray(y_vals, dtype=np.float64)
        A[32:48] = np.asarray(z_vals, dtype=np.float64)
        An = (A - self.gmin) / self.grng
        return An[0:16].copy(), An[16:32].copy(), An[32:48].copy()
    ...
    def sensor_callback(self, msg):
        try:
            x_raw = [t.x for t in msg.sensors[0].taxels]
            y_raw = [t.y for t in msg.sensors[0].taxels]
            z_raw = [t.z for t in msg.sensors[0].taxels]

            x_vals, y_vals, z_vals = self._normalize_taxels(x_raw, y_raw, z_raw)  # 모두 float64
            ...
            vecs = np.array([self.cum_x, self.cum_y, self.cum_z], dtype=np.float64).T  # (16,3)
            norms = np.linalg.norm(vecs, axis=1)
            max_norm = float(np.max(norms)) if np.isfinite(norms).all() else 0.0
            if max_norm <= 0.0:
                max_norm = 1e-6
            ...
            mean_vec_biased = np.sum(vecs, axis=0) / denom  # (3,)

            # --- 휴리스틱 ---
            # self.z_aligned = False
            # self.contacted = False
            norm = float(np.linalg.norm(mean_vec_biased))
            z_axis = np.array([0.0, 0.0, 1.0], dtype=np.float64)
            cos_theta = float(np.dot(mean_vec_biased, z_axis) / (norm + 1e-8))
    ...
    def overcome(self):
        ...
        while True:
            ...
            # self.z_aligned = True ####
            self._log_state_table("pre-grasp-check")
            if self.z_aligned:
...
########################### 슬립실험 ###############################################################
    ...
    #         self.contacted_ff = False
    #         self.contacted_kk = False
    #         print("초기화함초기화함초기화함초기화함초기화함초기화함초기화함초기화함초기화함초기화함")
...
# ============================ 책상 ========================================== #
    ...
    def execute_sequence(self):
        ...
        while rclpy.ok():
            ...
            while not self.goal(self.ba1_up, 'keep'): 
                    self.get_logger().warn("  >> 실패 → 재시도 중")

            self.reset_sensor_data()
            
            while not self.goal(self.ba1, 'keep'): 
                    self.get_logger().warn("  >> 실패 → 재시도 중")
            ...
            # while not self.goal(self.edge_up, 'keep'): 
            #         self.get_logger().warn("  >> 실패 → 재시도 중")
            ...
            # while not self.goal(self.ba2, 'keep'): 
            #         self.get_logger().warn("  >> 실패 → 재시도 중")
