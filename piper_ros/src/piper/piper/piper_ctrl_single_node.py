...
from std_msgs.msg import Int8  # noqa: F401
import time
import threading
import argparse
import math
...
class PiperRosNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.create_subscription(JointState, 'joint_ctrl_single_clik', self.joint_clik_callback,1)
        self.create_subscription(Bool,'mode_', self.mode_change, 1)
...
##################################################################################################################
    ...
    def mode_change(self, mode_):
        if mode_.data == False:
            self.mode = False
            self.get_logger().info(f"mode moveit")
        else:
            self.mode = True
            self.get_logger().info(f"mode CLIK")
    ...
    def joint_clik_callback(self, joint_data):
        """Callback function for joint angles
        ...
        if self.mode == True:
            ...
            # self.get_logger().info(f"joint ctrl single 받음 : {joint_data}")
            factor = 57324.840764  # 1000*180/3.14
            # self.get_logger().info(f"Received moveit Joint States:")
